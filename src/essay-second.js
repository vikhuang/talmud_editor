...
export const Essay3 = forwardRef((props, ref) => {
    const onFocusedBoolean = (props.onFocused === 2);

    return(
        <div className='essay3'>
            <div className='essay3_block_right' style={{shapeOutside: `inset(${35}% ${0}px ${10}% ${0}px)`}}></div>
              <Editor essayNo={{no: 2, layer: 2, align: 'left'}} 
                onFocused={onFocusedBoolean}/>  
        </div>
    );
});

export const Essay4 = forwardRef((props, ref) => {
    const onFocusedBoolean = (props.onFocused === 3);

    return(
        <div className='essay3'>
            <div className='essay3_block_left' style={{shapeOutside: `inset(${35}% ${0}px ${10}% ${0}px)`}}></div>
                <Editor essayNo={{no: 3, layer: 2, align: 'right'}} 
                    onFocused={onFocusedBoolean}/> 
        </div>
    );
});
